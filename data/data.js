import Phrase from "../models/phrases";

export const PHRASES = [
  new Phrase(
    1,
    "Pedir suco",
    "Pode me trazer um suco de laranja, por favor?",
    "May I have an orange juice, please?",
    require("../assets/audios/suco.wav")
  ),
  new Phrase(
    2,
    "Agradecer formal",
    "Obrigado",
    "Thank you",
    require("../assets/audios/obrigado.wav")
  ),
  new Phrase(
    3,
    "Dizer bom dia.",
    "Bom dia",
    "Good morning",
    require("../assets/audios/bomdia.wav")
  ),  
  new Phrase(
    4,
    "Dizer seu nome.",
    "Meu nome é Otto.",
    "My name is Otto",
    require("../assets/audios/meunome.wav")
  ),
  new Phrase(
    5,
    "Perguntar o nome de alguém.",
    "Qual é o seu nome?",
    "What is your name?",
    require("../assets/audios/nome.wav")
  ),
  new Phrase(
    6,
    "Perguntar como a pessoa está.",
    "Como você está?",
    "How are you?",
    require("../assets/audios/comovai.wav")
  ),
  new Phrase(
    7,
    "Perguntar o preço.",
    "Quanto custa isso?",
    "How much is it?",
    require("../assets/audios/custo.wav")
  ),
  new Phrase(
    8,
    "Perguntar se alguém quer jogar.",
    "Você quer jogar?",
    "Would you like to play?",
    require("../assets/audios/jogar.wav")
  ),
  new Phrase(
    9,
    "Dizer a nacionalidade.",
    "Eu sou brasileiro.",
    "I am brazilian.",
    require("../assets/audios/nacionalidade.wav")
  ),
  new Phrase(
    10,
    "Pedir um sanduíche.",
    "Pode me trazer um mixto quente, por favor?",
    "Can I have a ham and cheese sandwich, please?",
    require("../assets/audios/mixto.wav")
  ),
  new Phrase(
    11,
    "Pedir água.",
    "Pode me trazer um copo de água, por favor?",
    "Can I have a glass of water, please?",
    require("../assets/audios/agua.wav")
  ),
  new Phrase(
    12,
    "Perguntar como foi o dia.",
    "Como foi o seu dia?",
    "How was your day?",
    require("../assets/audios/comofoidia.wav")
  ),
  new Phrase(
    13,
    "Dizer boa noite.",
    "Boa noite!",
    "Good night! / Good evening!",
    require("../assets/audios/boanoite.wav")
  ),
  new Phrase(
    14,
    "Pedir desculpas.",
    "Desculpe me.",
    "I'm sorry.",
    require("../assets/audios/desculpa.wav")
  ),
  new Phrase(
    15,
    "Pedir licença.",
    "Com licença.",
    "Excuse me.",
    require("../assets/audios/licenca.wav")
  ),
  new Phrase(
    16,
    "Vamos!",
    "Vamos lá.",
    "Come on!",
    require("../assets/audios/vamos.wav")
  ),
  new Phrase(
    17,
    "Dizer que tem que ir embora.",
    "Tenho que ir.",
    "I’d better be going.",
    require("../assets/audios/precisoir.wav")
  ),     
  new Phrase(
    18,
    "Dizer para alguém se cuidar.",
    "Cuide-se.",
    "Take care.",
    require("../assets/audios/cuida.wav")
  ),
  new Phrase(
    19,
    "Dizer boa tarde.",
    "Boa tarde!",
    "Good afternoon!",
    require("../assets/audios/boatarde.wav")
  ),
  new Phrase(
    20,
    "Despedir-se",
    "Até mais.",
    "See you!",
    require("../assets/audios/atemais.wav")
  ),
  new Phrase(
    21,
    "Tudo bem?",
    "Tudo bem?",
    "All right?",
    require("../assets/audios/tudobem.wav")
  ),
  new Phrase(
    22,
    "Agradecer informal",
    "Obrigado, amigo",
    "Cheers mate!",
    require("../assets/audios/obrigadoamigo.wav")
  ), 
  new Phrase(
    23,
    "Perguntar onde fica o banheiro.",
    "Com licença, onde fica o banheiro, por favor?",
    "Excuse me, where is the toilet, please?",
    require("../assets/audios/banheiro.wav")
  ),     
  new Phrase(
    24,
    "Boa!",
    "Boa!",
    "Nice one!",
    require("../assets/audios/boa.wav")
  ),       
  new Phrase(
    25,
    "Cumprimentar e dizer o nome.",
    "Olá, meu nome é Otto.",
    "Hello, my name is Otto.",
    require("../assets/audios/nomesaudacao.wav")
  ), 
  new Phrase(
    26,
    "Dizer a idade.",
    "Eu tenho 10 anos de idade.",
    "I am 10 years old.",
    require("../assets/audios/idade.wav")
  ),
  new Phrase(
    27,
    "Dizer onde mora.",
    "Eu moro em Portugal.",
    "I live in Portugal.",
    require("../assets/audios/moro.wav")
  ), 
  new Phrase(
    28,
    "Quando conhece alguém.",
    "Prazer em conhecê-la/o.",
    "Nice to meet you.",
    require("../assets/audios/conhece.wav")
  ),    
];
